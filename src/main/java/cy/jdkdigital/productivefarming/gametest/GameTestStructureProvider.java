...
/**
 * Generates the empty NBT structure template used as the gametest playground. Vanilla
 * {@code minecraft:empty} is 1×1×1; the farm multiblock needs room, so we ship a 9×9×9 box.
 * The contents are empty — each test body places the blocks it needs via {@code helper.setBlock}.
 */
public class GameTestStructureProvider implements DataProvider
{
    private static final Vec3i SIZE = new Vec3i(9, 9, 9);
    private static final String NAME = "empty_9x9";
    ...
    public CompletableFuture<?> run(CachedOutput cache) {
        Path path = pathProvider.file(Identifier.fromNamespaceAndPath(ProductiveFarming.MODID, NAME), "nbt");
    ...
    public String getName() {
        return "Productive Farming GameTest Structures";
    }
}
